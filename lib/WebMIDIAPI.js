...
//init: create plugin
  ...
  function MIDIAccess( successCallback, errorCallback ) {
      ...
      if (this._jazzInstances[0]._Jazz) {
        ...
        window.setTimeout( bind(this, _onReady), 3 );
      } else {
          if (errorCallback)
  ...
  function MIDIInput( midiAccess, target ) {
    ...
    this._input = this._jazzInstance.MidiInOpen( this._index, _midiProc );
  }
  ...
  MIDIInput.prototype.dispatchEvent = function (evt) {
    ...
    for (var i=0; i<this._listeners.length; i++)
      if (this._listeners[i].handleEvent)
        bind(this, this._listeners[i].handleEvent)( evt );
      else
        bind(this, this._listeners[i])( evt );
  ...
  MIDIOutput.prototype.send = function( data, timestamp ) {
    ...
    if (timestamp && (delayBeforeSend>1)) {
      ...
      window.setTimeout( bind(sendObj, _sendLater), delayBeforeSend );
    } else {
